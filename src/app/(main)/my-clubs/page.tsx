...
import {
  ...
  Bell, Settings, TrendingUp, Calendar, Star,
  MessageCircleReply, UserCheck, Megaphone,
} from 'lucide-react';
import { CLUBS } from '@/data/clubs';
import { MOCK_USER, MOCK_ACTIVITY } from '@/data/mockUser';
...
export default function MyClubsPage() {
  const joinedClubs    = CLUBS.filter((c) => MOCK_USER.joinedClubs.includes(c.id));
  const bookmarkedClubs = CLUBS.filter((c) => MOCK_USER.bookmarkedClubs.includes(c.id));
      ...
      <div className="bg-[var(--bg)] rounded-2xl border border-[var(--bdr)] overflow-hidden mb-6">
        ...
        <div className="px-6 pb-5">
          <div className="flex items-end gap-4 -mt-8 mb-4">
            ...
            <div className="mb-1">
              <h1 className="text-lg font-serif font-bold text-[var(--txt)]">{MOCK_USER.name}</h1>
              <p className="text-xs text-[var(--txt3)]">{MOCK_USER.department} · {MOCK_USER.grade}학년 · {MOCK_USER.email}</p>
            </div>
          ...
          <div className="flex flex-wrap gap-4">
            {[
              ...
              { icon: <BookOpen size={13} />,   value: 3,                      label: '작성 게시글' },
              { icon: <MessageSquare size={13} />, value: 7,                   label: '작성 댓글' },
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className="text-[10px] text-[var(--txt3)] bg-[var(--bg2)] px-2 py-0.5 rounded-full">{CATEGORY_LABEL[club.category]}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-3 mt-1.5 text-[10px] text-[var(--txt3)]">
                        ...
                        {club.isRecruiting && (
                          <span className="text-emerald-600 font-medium">모집 중</span>
                        )}
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-2 flex-shrink-0">
                      <Link
                        href={`/clubs/${club.id}/board/write`}
          ...
          <section className="bg-[var(--bg)] rounded-2xl border border-[var(--bdr)] overflow-hidden">
            ...
            <div className="divide-y divide-[var(--bdr)]">
              {MOCK_ACTIVITY.map((a) => (
                ...
                >
                  <div className={cn('w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5', ACTIVITY_COLOR[a.type])}>
                    {ACTIVITY_ICON[a.type]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-[var(--txt)] leading-snug">{a.content}</p>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="text-[10px] font-medium text-indigo-500">{a.clubName}</span>
                      <span className="text-[10px] text-[var(--txt3)]">· {a.time}</span>
                    </div>
                  </div>
                </Link>
              ))}
          ...
          <div className="bg-[var(--bg)] rounded-2xl border border-[var(--bdr)] p-4">
            ...
            <div className="space-y-1">
              {[
                ...
                { href: '/clubs/chaos/board/write',  label: 'CHAOS에 글 쓰기',  icon: <BookOpen size={13} /> },
