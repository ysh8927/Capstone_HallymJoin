...
export default function Header() {
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-1">
            ...
            <div ref={userRef} className="relative hidden sm:block">
              ...
              >
                ...
                <span className="text-xs font-medium text-[var(--txt)] max-w-[60px] truncate">{MOCK_USER.name}</span>
              ...
              {userOpen && (
                <div className="absolute right-0 top-full mt-2 w-52 bg-[var(--bg)] rounded-2xl border border-[var(--bdr)] shadow-xl z-50 overflow-hidden animate-fade-up">
                  ...
                  <div className="px-4 py-3 border-b border-[var(--bdr)]">
                    <p className="text-xs font-semibold text-[var(--txt)]">{MOCK_USER.name}</p>
                    <p className="text-[11px] text-[var(--txt3)] mt-0.5">{MOCK_USER.department} {MOCK_USER.grade}학년</p>
                    <p className="text-[11px] text-[var(--txt3)]">{MOCK_USER.email}</p>
                  </div>
                  {/* Menu */}
                  <div className="p-2">
                    <Link
                    ...
                    <Link
                      href="/login"
                      onClick={() => setUserOpen(false)}
                      className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs text-rose-500 hover:bg-rose-50 transition-colors"
                    >
                      <LogOut size={13} /> 로그아웃
                    </Link>
                  </div>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-[var(--bdr)] bg-[var(--bg)] px-4 py-3 flex flex-col gap-1 animate-fade-in">
            ...
            <div className="flex items-center gap-3 px-3 py-2.5 mb-1 bg-[var(--bg2)] rounded-xl">
              ...
              <div>
                <p className="text-sm font-semibold text-[var(--txt)]">{MOCK_USER.name}</p>
                <p className="text-[11px] text-[var(--txt3)]">{MOCK_USER.department}</p>
              </div>
            </div>
            {NAV_LINKS.map(({ href, label }) => (
              <Link
            ...
            <div className="pt-2 border-t border-[var(--bdr)]">
              <Link href="/login" className="flex items-center gap-2 text-sm py-2 px-3 rounded-lg text-rose-500 hover:bg-rose-50" onClick={() => setMenuOpen(false)}>
                <LogOut size={14} /> 로그아웃
              </Link>
            </div>
          </div>
      ...
      {/* Search modal (portal-like, outside sticky header) */}
